...
function Row({
  ...
  return (
    <div
      className={`h-full w-full rounded-2xl bg-white/80 p-4 dark:bg-white/[0.04] ${
        compact ? "min-h-[112px]" : "min-h-[152px]"
      }`}
    >
      ...
      <p
        className={`mt-2 text-sm text-slate-700 dark:text-white/76 ${
  ...
  return (
    <section className="w-full rounded-[2rem] border border-slate-200 bg-slate-50/72 p-6 dark:border-white/10 dark:bg-white/5 lg:p-8 xl:min-h-[620px]">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <Row label="Target customer" value={prospect.target_customer} compact />
        <Row label="Best persona recommendation" value={prospect.best_sales_persona} compact />
