  ...
  return (
    <>
      <div className="flex w-full flex-col items-stretch px-4 md:w-1/2 lg:w-1/3">
        <div
          className={`relative z-10 mb-10 flex h-full flex-col overflow-hidden rounded-2xl bg-white px-8 py-10 transition duration-300 hover:shadow-2xl dark:bg-dark sm:p-12 lg:px-8 lg:py-12 xl:p-12 ${
            isHighlighted
              ? "z-20 border border-primary shadow-xl md:scale-105 lg:scale-[1.03] dark:border-accent "
        ...
        >
          ...
          {planBadge && (
            <>
              <p className="absolute right-4 top-4 inline-block rounded-full bg-primary px-4 py-1 text-xs font-semibold text-white sm:hidden">
                {planBadge}
              </p>
              <p className="absolute -right-8 top-[60px] hidden rotate-45 rounded-md bg-primary px-8 py-1.5 text-xs font-semibold text-white sm:inline-block">
                {planBadge}
              </p>
            </>
          )}
          <span className="mb-3 block text-sm font-bold uppercase tracking-[0.22em] text-muted dark:text-accent">{product.nickname}</span>
          <h2 className="display text-4xl font-extrabold text-ink dark:text-white xl:text-[40px] xl:leading-[1.1]">{priceDisplay}</h2>
          <p className="mt-4 text-sm leading-7 text-muted dark:text-white/65">{planDescriptions[planName]}</p>
          <div className="mb-8 mt-8 grow">
            <h3 className="mb-6 border-b border-black/8 pb-4 text-base font-semibold text-ink dark:border-white/10 dark:text-white">
              Included Features
            </h3>
            <div className="mb-8 text-muted dark:text-white/70">
          ...
          <button
            ...
            className={`mt-auto inline-block w-full rounded-full px-7 py-3.5 text-center text-sm font-semibold transition-all duration-300 ${
              buttonConfig.disabled
                ? "cursor-not-allowed border border-black/8 bg-black/[0.04] text-muted dark:border-white/10 dark:bg-white/5 dark:text-white/40"
                : isHighlighted
                  ? "bg-primary text-white hover:bg-primary-deep"
                  : "border border-black/8 bg-black/[0.04] text-ink hover:bg-black/[0.07] dark:border-white/10 dark:bg-white/10 dark:text-white dark:hover:bg-white/20"
            }`}
          >
            {buttonConfig.text}
          </button>
