...
const socialLinks = ({ facebookLink, twitterLink, instagramLink }: TeamType) => [
  { href: facebookLink, icon: socialIcons.facebook },
  { href: twitterLink, icon: socialIcons.twitter },
  { href: instagramLink, icon: socialIcons.instagram },
];

const DotPattern = () => (
  <svg width="55" height="53" viewBox="0 0 55 53" fill="none">
    {Array.from({ length: 30 }, (_, index) => {
      const cx = 2.52045 + (index % 6) * 9.99135;
      const cy = 3.1009 + Math.floor(index / 6) * 9.9799;
      return <circle key={index} cx={cx} cy={cy} r="1.55045" fill="#3758F9" />;
    })}
  </svg>
);
    ...
    <div className="w-full px-4 sm:w-1/2 lg:w-1/4 xl:w-1/4">
      <div className="group mb-8 rounded-xl bg-white px-5 pb-10 pt-12 shadow-testimonial dark:bg-dark dark:shadow-none">
        ...
        <div className="text-center">
          ...
          <p className="mb-5 text-sm text-body-color dark:text-dark-6">{designation}</p>
          <div className="flex items-center justify-center gap-5">
            {socialLinks(team).map((link) => (
              <Link key={link.href} aria-label="social link" href={link.href} className="text-dark-6 hover:text-primary">
                {link.icon}
              </Link>
            ))}
          </div>
